...
def calculate_consequences(
    ...
    # do it!
    # 'results' dict gets updated by '_calculate_consequences'
    ep_models = []
    for transcript in transcripts:
        ...
        try:
            _calculate_consequences(ep_models[-1])
    ...
    return \
        EffectPredictionResults(
            variant_orig,
            [m.result(most_severe=most_severe) for m in ep_models]
        )


def _calculate_consequences(ep_model):
    build = ep_model.genome_build
    transcript = ep_model.transcript
    variant = ep_model.variant

    # always check!
    # ref == alt?
    for c in [
        consequences.no_sequence_alteration
    ]:
        result = \
            c.get('function')(variant, transcript)
        ep_model.add_result(c, result, stop=True)

    # non-coding transcripts don't have features,
    for c in [
        consequences.non_coding_transcript_variant,
        consequences.non_coding_transcript_exon_variant
    ]:
        result = \
            c.get('function')(variant, transcript)
        ep_model.add_result(c, result)

    # upstream/downstream?
    for c in [
        consequences.upstream_transcript_variant,
    ...
    if exons:
        ep_model.update_context(exons=exons)
    for exon in exons:
        for c in [
            consequences.exon_loss
        ]:
    ...
    if utr:
        for c in [
            consequences.five_prime_utr_variant,
            consequences.three_prime_utr_variant
        ]:
            result = \
                c.get('function')(variant, transcript, utr)
            ep_model.add_result(c, result)

    # elongation/truncation
    for c in [
        consequences.feature_elongation_variant,
    ...
    if cds:
        ep_model.update_context(cdses=[cds])

    # is intronic??
    intron = find_overlapping(transcript, 'intron', variant)
    if intron:
        ep_model.update_context(introns=[intron])
        for c in [
            consequences.intronic_variant,
        ]:
            result = \
                c.get('function')(variant, transcript)
            ep_model.add_result(c, result)
    ...
    ep_model.calculate_splicing_effects()
    ...
    #     ep_model.add_result(consequences.intronic_variant, True)
    ...
    codons_ref_seq = \
        split_into_codons(builder_result.reference_sequence.coding_sequence)
    codons_var_seq = \
        split_into_codons(builder_result.alternate_sequence.coding_sequence)

    ep_model.update_context(
    ...
    for c in [
        # initiator/start
        consequences.initiator_codon_variant,
    ...
    for c in [
        ...
        consequences.incomplete_terminal_codon_variant,
    ...
    if coding_variant:
        ...
        for c in [
            consequences.inframe_deletion_variant,
    ...
    for c in [
        # synonymous/missense
        consequences.coding_sequence_variant,
    ...
    for c in [
        consequences.stop_gained_variant
    ]:
